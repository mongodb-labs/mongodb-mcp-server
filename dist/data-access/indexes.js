...
export function registerIndexesTools(server, globalState) {
    server.tool("list-indexes", "Describe the indexes for a collection", {
    ...
    server.tool("create-index", "Create an index for a collection", {
        collection: z.string().describe("Collection name"),
        database: z.string().describe("Database name"),
        keys: z.record(z.string(), z.custom()).describe("The index definition"),
    }, async ({ database, collection, keys }) => {
        const provider = ensureConnected(globalState);
